...
class DydxPerpetualOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self,
                 trading_pairs: Optional[List[str]] = None, domain: str = "dydx_perpetual"):
        super().__init__(data_source=DydxPerpetualAPIOrderBookDataSource(trading_pairs=trading_pairs, domain=domain),
    ...
    def data_source(self) -> OrderBookTrackerDataSource:
        if not self._data_source:
            if self._data_source_type is OrderBookTrackerDataSourceType.REMOTE_API:
                ...
                self._data_source = DydxPerpetualAPIOrderBookDataSource(trading_pairs=self._trading_pairs)
            else:
    ...
    async def _order_book_diff_router(self):
        ...
        while True:
            try:
                ...
                if int(order_book.snapshot_uid) > int(ob_message.update_id):
                    messages_rejected += 1
                    continue
